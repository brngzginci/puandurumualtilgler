...
const DEFAULT_CONFIG: DesignConfig = {
  ...
  zoneDefinitions: getDefaultZoneDefinitions("tff-1-lig", 20, "overall"),
...
export default function App() {
  ...
  const [config, setConfig] = useState<DesignConfig>(() => {
    ...
    const targetZones = getDefaultZoneDefinitions(selection.leagueId, standings.length, selection.groupId);
    const targetBounds = getDefaultZoneBounds(selection.leagueId, standings.length, selection.groupId);
    ...
    if (saved) {
      try {
        ...
        // If parsed has outdated zones from previous test formats, update with official defaults
        const hasOutdatedZones =
          !parsed.zoneDefinitions ||
          parsed.zoneDefinitions.some(
            (z: any) =>
              z.label?.includes("Çeyrek") ||
              (selection.leagueId === "tff-3-lig" && z.id === "relegation" && z.startPosition === 16)
          );
        const resolvedZones = hasOutdatedZones ? targetZones : parsed.zoneDefinitions;

        return {
          ...DEFAULT_CONFIG,
          ...targetBounds,
          ...parsed,
          zoneDefinitions: resolvedZones
        };
      } catch (e) {
  ...
  const handleSelectCompetition = (
    ...
    const targetZones = getDefaultZoneDefinitions(targetLeagueId, targetTeamCount, targetGroupId);
    const targetBounds = getDefaultZoneBounds(targetLeagueId, targetTeamCount, targetGroupId);
  ...
  const handleResetData = () => {
    ...
    const targetZones = getDefaultZoneDefinitions(selection.leagueId, samples.length, selection.groupId);
    const targetBounds = getDefaultZoneBounds(selection.leagueId, samples.length, selection.groupId);
            ...
            {activeTab === "zones" && (
              <ZoneEditor
                ...
                zoneDefinitions={
                  ...
                  getDefaultZoneDefinitions(selection.leagueId, standings.length, selection.groupId)
                }
                onChange={(newZones) => {
                  setConfig((prev) => ({
                    ...prev,
