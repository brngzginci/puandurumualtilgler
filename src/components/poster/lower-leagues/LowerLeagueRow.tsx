...
export const LowerLeagueRow: React.FC<LowerLeagueRowProps> = ({
  ...
  if (zones && Array.isArray(zones) && zones.length > 0) {
    ...
    if (position >= (config.directPromotionStart ?? 1) && position <= (config.directPromotionEnd ?? 1)) {
      rankBg = "#128C08";
  ...
  return (
    ...
    >
      ...
      <div className="lower-league-col-team">
        ...
        <span className="lower-league-team-name">{teamDisplayName}</span>
      </div>

      {/* Stats Columns: O, G, B, M, AG, YG, AV, P */}
      <div className="lower-league-col-stats">
        <div className="lower-league-stat-cell lower-league-stat-val">{row.played ?? row.matchesPlayed ?? 0}</div>
        <div className="lower-league-stat-cell lower-league-stat-val">{row.won ?? 0}</div>
        <div className="lower-league-stat-cell lower-league-stat-val">{row.drawn ?? 0}</div>
        <div className="lower-league-stat-cell lower-league-stat-val">{row.lost ?? 0}</div>
        <div className="lower-league-stat-cell-lg lower-league-stat-val">{row.goalsFor ?? 0}</div>
        <div className="lower-league-stat-cell-lg lower-league-stat-val">{row.goalsAgainst ?? 0}</div>
        ...
        <div className="lower-league-stat-cell-points lower-league-stat-val-pts">{row.points ?? 0}</div>
      </div>
    </div>
  );
};
